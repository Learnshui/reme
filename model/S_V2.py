...
class S_V2_Net(nn.Module):
    def __init__(self,out_planes=1,ccm=True,norm_layer=nn.BatchNorm2d,is_training=True,expansion=2,base_channel=32):
        ...
        self.conv3x3_1=nn.Conv2d(32,32,kernel_size=3,stride=1,padding=1)
        self.conv3x3_2=nn.Conv2d(16,16,kernel_size=3,stride=1,padding=1)
        self.conv3x3_3=nn.Conv2d(8,8,kernel_size=3,stride=1,padding=1)
        ...
        self.expand2 = nn.Sequential(nn.Conv2d(1, 64, kernel_size=1),
                                    nn.BatchNorm2d(64),
                                    nn.ReLU(inplace=True))
        self.expand3 = nn.Sequential(nn.Conv2d(1, 128, kernel_size=1),
                                    nn.BatchNorm2d(128),
                                    nn.ReLU(inplace=True)) 
        self.expand4 = nn.Sequential(nn.Conv2d(1, 256, kernel_size=1),
                                    nn.BatchNorm2d(256),
                                    nn.ReLU(inplace=True))
        ...
        self.d_0 = nn.Conv2d(64, 32, kernel_size=1)
        self.c_3 = nn.Conv2d(64, 1, kernel_size=1)
        self.d_2 = nn.Conv2d(32, 16, kernel_size=1)
        self.d_3 = nn.Conv2d(16, 8, kernel_size=1)
        self.c_4 = nn.Conv2d(128, 1, kernel_size=1)
        self.c_5 = nn.Conv2d(256, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        c1 = self.backbone.relu(x)#1/2  64    torch.Size([1, 64, 112, 112])
        
        x = self.backbone.maxpool(c1)#torch.Size([1, 64, 56, 56])
        c2 = self.backbone.layer1(x)#1/4   64   torch.Size([1, 64, 56, 56])
        c3 = self.backbone.layer2(c2)#1/8   128  
        c4 = self.backbone.layer3(c3)#1/16   256  
        c5 = self.backbone.layer4(c4)#1/32   512      torch.Size([1, 512, 7, 7])
        #edge
        ss = F.interpolate(self.d_0(c1), x_size[2:],  #224,224,32
                            mode='bilinear', align_corners=True)
        ss = self.conv3x3_1(ss)
        c_3 = F.interpolate(self.c_3(c2), x_size[2:],
                            mode='bilinear', align_corners=True)#224,224,1
        ss1 = self.gate1(ss, c_3)#torch.Size([1, 32, 224, 224])
        # ss1 = self.conv3x3_2(ss1)
        ss = self.d_2(ss1)#torch.Size([1, 16, 224, 224])
        c_4 = F.interpolate(self.c_4(c3), x_size[2:],
                            mode='bilinear', align_corners=True)
        ss2 = self.gate2(ss, c_4)#torch.Size([1, 16, 224, 224])
        # ss2 = self.conv3x3_3(ss2)
        ss3 = self.d_3(ss2)
        c_5 = F.interpolate(self.c_5(c4), x_size[2:],
                            mode='bilinear', align_corners=True)
        ss3 = self.gate3(ss3, c_5)#torch.Size([1, 8, 224, 224])
        ss = self.fuse(ss3)#torch.Size([1, 1, 224, 224])
        # ss = F.interpolate(ss, x_size[2:], mode='bilinear', align_corners=True)
        edge_out = self.sigmoid(ss)#1,1,256,256
        #
        edge1=self.expand1(edge_out)#torch.Size([1, 64, 224, 224])
        edge2=self.expand2(edge_out)#torch.Size([1, 64, 224, 224])
        edge3=self.expand3(edge_out)
        edge4=self.expand4(edge_out)

        edge1 = F.interpolate(edge1, scale_factor=1/2, mode='bilinear')#torch.Size([1, 32, 112, 112])
        ...
        m2=self.mce_2(c2)
        m3=self.mce_3(c3)
        m4=self.mce_4(c4)#torch.Size([1, 256, 14, 14])
        # d_bottom=self.bottom(c5)
        c5=self.sap(c5)

        # d5=d_bottom+c5           #512

        d4=self.relu(self.decoder5(c5)+m4+edge4)  #256
        d3=self.relu(self.decoder4(d4)+m3+edge3)  #128
        d2=self.relu(self.decoder3(d3)+m2+edge2) #64  torch.Size([1, 64, 56, 56])
        d1=self.decoder2(d2)+c1+edge1 #32
